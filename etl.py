...
def process_song_data(spark, input_data, output_data):
    '''
    Loads the data from s3 and writes the dataframe processed for songs and artists back on s3
    '''
    ...
    df =  df = spark.read.json(song_data, schema=songSchema)
...
def process_log_data(spark, input_data, output_data):
    '''
    Loads log data from s3 and writes the processed dataframe  back on s3
    '''
    ...
    songplays = artists_songs_logs.join(
        time_table,
        artists_songs_logs.ts == time_table.start_time, 'left').drop(artists_songs_logs.year)
